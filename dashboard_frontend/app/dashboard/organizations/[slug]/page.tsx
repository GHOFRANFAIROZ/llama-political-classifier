...
export default function OrgDashboardPage() {
  ...
  const foundOrg = useMemo(() => {
    ...
    // 2) slug fallback من id
    found = orgs.find((o) => (o.slug || slugifyFallback(o.id)) === slug);
    if (found) return found;

    // 3) إذا الرابط كان org_id
    found = orgs.find((o) => o.id === slug);
    return found || null;
  }, [orgs, slug]);

  // مزامنة الـ slug مع OrgContext
  useEffect(() => {
    if (orgsLoading) return; // ✅ لا تحكم “not found” قبل ما تخلص
    setOrg(foundOrg);

    if (foundOrg && (!currentOrg || currentOrg.id !== foundOrg.id)) {
      setCurrentOrg(foundOrg);
    }
    // نستخدم currentOrg?.id لتجنب rerenders غير ضرورية
  }, [orgsLoading, foundOrg, currentOrg?.id, setCurrentOrg]);

  // تحميل stats / trends / wordcloud
  useEffect(() => {
    if (!org) return;
    ...
    async function fetchAll() {
      ...
      try {
        ...
        if (!statsRes.ok || !trendsRes.ok || !wordcloudRes.ok) {
          throw new Error("Backend error while loading org data");
        }
        ...
        setStats({
          totalReports: statsJson.totalReports ?? 0,
          last7dReports: statsJson.last7dReports ?? 0,
          activeUsers: statsJson.activeUsers ?? null,
          hateSpeechRatio: statsJson.hateSpeechRatio ?? 0,
          mostToxicPlatform: statsJson.mostToxicPlatform ?? null,
          timeToFirstReviewHours: statsJson.timeToFirstReviewHours ?? null,
        });

        setTrendsSeries(trendsJson.timeseries ?? []);
        setPlatformSeries(trendsJson.byPlatform ?? []);
        setWordcloudTerms(wordcloudJson.terms ?? []);
      } catch (err: any) {
        if (err.name === "AbortError") return;
        setError(err.message || "Failed to load org data");
      } finally {
        setLoading(false);
      }
    }
  ...
  }, [org?.id, dateRange]); // ✅ ربط واضح على org.id بدل org object كله

  // ✅ شاشة تحميل قبل orgs ما تخلص
  if (orgsLoading) {
    return (
  ...
  // ✅ بعد ما يخلص loading: لو ما لقينا org
  if (!org) {
    return (
    ...
    >
      ...
      <div className="flex flex-wrap justify-between gap-4 items-start">
        <div className="space-y-2">
          ...
          <p className="text-purple-400">
            Organization-specific dashboard powered by Org Mode (Firestore).
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={stats ? `${hateRatioPercent.toString()}%` : "—"}
          loading={loading}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Trends */}
        <div className="lg:col-span-2 bg-[#120F18] border border-purple-900/60 rounded-2xl p-5 shadow-[0_0_18px_rgba(176,92,255,0.25)] h-[320px]">
        ...
        {/* By platform */}
        <div className="bg-[#120F18] border border-purple-900/60 rounded-2xl p-5 shadow-[0_0_18px_rgba(176,92,255,0.25)] h-[320px]">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-sm font-semibold text-purple-100">
      ...
      <div className="bg-[#120F18] border border-purple-900/60 rounded-2xl p-5 shadow-[0_0_18px_rgba(176,92,255,0.25)]">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-sm font-semibold text-purple-100">
            Targeted groups & entities (wordcloud)
          </h2>
          <span className="text-[11px] text-purple-400">
